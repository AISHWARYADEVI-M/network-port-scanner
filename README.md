...
Multithreading (threading module) – Fast scanning
Queue Module – Thread-safe communication
Time Module – Performance tracking
ChatGPT (LLM-based Assistance) – Used for development guidance, debugging, and documentation
🚀 Installation
...
8080	HTTP-Alt
🧠 How It Works
The scanner attempts to establish a TCP connection using socket.connect_ex()
If the connection is successful → port is marked open
Ports are mapped to known services using a predefined dictionary
Multithreading speeds up the scanning process
Results are displayed in real time via GUI
👥 End Users
...
Use it only on systems you own or have explicit permission to scan. Unauthorized scanning may be illegal.
